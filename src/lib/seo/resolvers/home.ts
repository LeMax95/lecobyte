import { getHomeSeoConfig } from "@/lib/seoContent";
...
export async function resolveHomeSeo(): Promise<ResolvedSeoPage> {
  const [{ site, ids, schema }, seoConfig] = await Promise.all([
    resolveSiteSeo(),
    getHomeSeoConfig(),
  ]);

  const canonical = buildCanonical(site.url, "/");
