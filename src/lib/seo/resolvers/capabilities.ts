import { getCapabilitiesSeoConfig } from "@/lib/seoContent";
...
export async function resolveCapabilitiesSeo(): Promise<ResolvedSeoPage> {
  const [{ site, ids, schema: siteSchema }, seoConfig] = await Promise.all([
    resolveSiteSeo(),
    getCapabilitiesSeoConfig(),
  ]);

  const canonical = buildCanonical(site.url, "/capabilities");
  ...
  const breadcrumbs = [
    ...
    { name: "Capabilities", url: canonical },
  ];
