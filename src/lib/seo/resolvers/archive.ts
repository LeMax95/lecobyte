import { getArchiveSeoConfig } from "@/lib/seoContent";
...
export async function resolveArchiveSeo(): Promise<ResolvedSeoPage> {
  const [{ site, ids, schema: siteSchema }, seoConfig] = await Promise.all([
    resolveSiteSeo(),
    getArchiveSeoConfig(),
  ]);

  const canonical = buildCanonical(site.url, "/archive");
